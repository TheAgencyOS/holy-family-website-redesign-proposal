...
| File | Role | Edit it? |
|---|---|---|
| **`index.html`** | **The proposal.** 30 paginated 8.5×11 pages with sidebar nav, all content, all design. This is what the live URL serves and what the PDF is generated from. | **Yes — only this.** |
| `HFU-Proposal.pdf` | Generated artifact. A render of `index.html` to PDF. | No — regenerate (see below). |
...
### Regenerating the PDF (after content changes are stable)
...
The print CSS in `index.html` enforces `height: 11in; overflow: hidden` per `.page` so each section maps to exactly one Letter sheet. Expect 30 PDF pages from 30 `<section class="page">` blocks. If the count differs, content is overflowing and needs trimming — do not "fix" it by changing the print CSS.
...
- **The PDF is downstream.** Never edit the PDF directly. If the PDF is stale, regenerate it.
