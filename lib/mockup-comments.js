...
.hfm-nudge {
  ...
  line-height: 1.45;
  box-shadow:
    0 10px 28px -8px rgba(15, 17, 23, 0.35),
    0 4px 10px -4px rgba(15, 17, 23, 0.22);
  opacity: 0;
  transform: translateY(-4px);
  transition: opacity .24s ease, transform .28s cubic-bezier(.2,.9,.3,1.1), visibility .24s;
  visibility: hidden;
  pointer-events: none;
}
.hfm-nudge.is-open {
...
@media (max-width: 560px) {
  .hfm-nudge { max-width: 240px; font-size: 12px; padding: 9px 28px 9px 12px; }
}
